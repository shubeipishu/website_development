...
`;
    ...
    loadGraph(input) { const file = input.files[0]; if (!file) return; const reader = new FileReader(); reader.onload = (e) => { try { const data = JSON.parse(e.target.result); if (data.nodes && data.edges) { this.app.saveState(); this.app.nodes = data.nodes; this.app.edges = data.edges; this.app.nodeIdSeq = data.nextId || (this.app.nodes.length > 0 ? Math.max(...this.app.nodes.map(n => n.id)) + 1 : 0); this.app.clearSel(); this.app.resetView(); this.app.algorithms.updateData(); alert("Data Loaded"); } } catch (err) { alert("Invalid File"); } input.value = ''; }; reader.readAsText(file); }
}
